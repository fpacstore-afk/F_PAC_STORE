...
import { 
  Sparkles, Check, RotateCw, ZoomIn, Eye, ShoppingBag, 
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] text-neutral-900 pt-4 pb-20 px-3 md:px-8 font-sans selection:bg-[#eab308] selection:text-black">
      <div className="max-w-7xl mx-auto mb-6 text-center">
        ...
        <p className="text-xs md:text-sm text-neutral-600 max-w-2xl mx-auto font-medium">Monte sua camiseta exclusiva passo a passo. Escolha a cor nobre, tamanho, estampas do catálogo e posicionamento com visualização em tempo real.</p>
      </div>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-6 bg-white border border-neutral-200 p-5 md:p-8 rounded-3xl shadow-xl flex flex-col justify-between min-h-[580px] text-neutral-900">
          ...
          {currentStep === 3 && <div className="space-y-4 animate-in fade-in duration-300"><div><span className="text-[10px] font-black text-[#d97706] uppercase tracking-[0.2em] font-mono">PASSO 3 DE 7</span><h2 className="text-xl md:text-2xl font-black uppercase text-neutral-900 mt-1">Escolha as Estampas do Catálogo</h2><p className="text-xs text-neutral-600 mt-1">Selecione uma estampa para configurar posição e tamanho.</p></div><div className="relative"><Search size={16} className="absolute left-3.5 top-3.5 text-neutral-400" /><input type="text" placeholder="Buscar estampa..." value={stampSearch} onChange={(e) => setStampSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-neutral-50 border border-neutral-200 rounded-xl text-xs text-neutral-900 placeholder-neutral-400 focus:outline-none focus:border-[#d97706]" /></div><div className="max-h-[320px] overflow-y-auto pr-1 grid grid-cols-2 sm:grid-cols-3 gap-3 custom-scrollbar">{loadingStamps ? <div className="col-span-full py-12 text-center text-neutral-500 text-xs"><RefreshCw size={24} className="animate-spin mx-auto mb-2 text-[#d97706]" />Carregando catálogo...</div> : filteredStamps.length === 0 ? <div className="col-span-full py-10 text-center text-neutral-500 text-xs bg-neutral-50 border border-neutral-200 rounded-xl p-4">Nenhuma estampa encontrada.</div> : filteredStamps.map((st) => <div key={st.id} onClick={() => { setActiveStampForPlacement(st); const allowedForStamp = Array.isArray(st.allowedLocations) && st.allowedLocations.length > 0 ? PRINT_POSITIONS.filter(pos => { const allowed = st.allowedLocations!.map(l => l.toLowerCase()); const posLabelLower = pos.label.toLowerCase(); const posIdLower = pos.id.toLowerCase(); return allowed.some(a => posLabelLower.includes(a) || a.includes(posLabelLower) || posIdLower.includes(a) || a.includes(posIdLower)); }) : PRINT_POSITIONS; const initialPos = allowedForStamp.length > 0 ? allowedForStamp[0] : PRINT_POSITIONS[1]; setConfiguringPosition(initialPos); setConfiguringSizeCm(initialPos.defaultSizeCm); setActiveViewSide(initialPos.viewSide); setRotationAngle(initialPos.viewSide === 'back' ? 180 : 0); setCurrentStep(4); }} className="bg-neutral-50 border border-neutral-200 hover:border-[#d97706] p-2 rounded-2xl group cursor-pointer transition-all duration-200 relative flex flex-col justify-between overflow-hidden"><div className="aspect-square w-full rounded-xl bg-neutral-100 overflow-hidden relative mb-2"><img src={st.image} alt={st.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" /><button onClick={(e) => toggleFavorite(st.id, e)} className="absolute top-1.5 right-1.5 p-1.5 bg-black/60 backdrop-blur-md rounded-full text-white hover:text-[#eab308]"><Star size={12} className={cn(favorites.includes(st.id) && 'fill-[#eab308] text-[#eab308]')} /></button></div><h4 className="text-[11px] font-black uppercase font-mono text-neutral-900 truncate">{st.code ? `SKU: ${st.code}` : st.name}</h4></div>)}</div></div>}
