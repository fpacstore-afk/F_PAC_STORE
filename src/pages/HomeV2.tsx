    ...
    <div className="w-full bg-white" data-home-version="canonical-v4">
      ...
      <section className="bg-black pt-[118px] md:pt-[146px]">
        <div className="relative w-full bg-black">
          ...
          <div className="px-5 py-7 md:py-10 flex flex-col items-center text-center bg-black border-t border-white/5">
            {brandImage ? (
              <img src={brandImage} alt="F PAC STORE" className="h-14 md:h-20 w-auto object-contain mb-3" />
            ) : (
              <h1 className="text-white text-4xl md:text-7xl font-black tracking-tight mb-2">F PAC STORE</h1>
            )}
            <p className="text-white/60 text-[10px] md:text-sm uppercase tracking-[0.4em] font-black mb-5">Estúdio de identidade</p>
      ...
      <section className="py-10 md:py-16">
        <div className="max-w-7xl mx-auto px-5 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-7">
          ...
          <div className="max-w-6xl mx-auto px-5">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {catalogImages.map((src, index) => (
                <div key={index} className="aspect-video bg-black overflow-hidden">
      ...
      {/* Coleções: só FORCE, MARK e PRIME. Nada de lista genérica ou loop infinito. */}
      <section id="collections" className="py-14 md:py-24 bg-white overflow-hidden">
        <div className="max-w-6xl mx-auto px-5">
              ...
              return (
                <Link key={slug} to={product ? getProductUrl(product) : `/model/${slug}`} className="snap-center shrink-0 w-[82vw] max-w-[360px] relative aspect-[4/5] overflow-hidden rounded-[2rem] bg-black shadow-xl">
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-7 text-center text-white">
                    ...
                    <p className="mt-2 text-white/70 text-[10px] uppercase tracking-widest font-bold">Toque para conhecer</p>
                  </div>
                </Link>
              );
            })}
          ...
          <div className="hidden md:block relative max-w-[560px] mx-auto">
            <button onClick={prev} aria-label="Coleção anterior" className="absolute -left-20 top-1/2 -translate-y-1/2 z-20 w-14 h-14 rounded-full bg-black/80 text-white flex items-center justify-center"><ChevronLeft /></button>
            ...
            <button onClick={next} aria-label="Próxima coleção" className="absolute -right-20 top-1/2 -translate-y-1/2 z-20 w-14 h-14 rounded-full bg-black/80 text-white flex items-center justify-center"><ChevronRight /></button>
