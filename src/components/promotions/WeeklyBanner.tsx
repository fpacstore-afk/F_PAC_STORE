...
export const WeeklyBanner: React.FC<WeeklyBannerProps> = ({
  ...
  const primaryColor = promotion.colors?.[0] || 'bg-black';
  const accentColor = promotion.colors?.[1] || '#eab308';
  const customBackgroundStyle = promotion.colors?.[0] ? { backgroundColor: promotion.colors[0] } : undefined;

  return (
    <div 
      onClick={handleBannerClick}
      style={customBackgroundStyle}
      className="relative w-full bg-black text-white py-12 md:py-16 px-6 cursor-pointer border-y border-white/5 select-none overflow-hidden transition-all duration-300 md:hover:border-white/10"
    >
      ...
      <div className="max-w-4xl mx-auto relative z-10 flex flex-col items-center text-center">
        ...
        <div 
          ...
          className="inline-flex items-center gap-1.5 bg-white/5 border px-3 py-1 rounded-full text-xs font-black uppercase tracking-widest mb-6"
        >
          <Sparkles size={12} className="animate-pulse" />
          <span>PROMOÇÃO EXCLUSIVA ATIVA</span>
        </div>

        {/* Title & Description */}
        <h2 
          style={{ color: accentColor }}
          className="text-2xl md:text-5xl font-black uppercase tracking-widest italic mb-4 max-w-2xl drop-shadow"
        >
          {promotion.title}
        </h2>
        
        <p className="text-gray-300 text-xs md:text-sm font-medium tracking-wider max-w-lg mb-8 uppercase">
          {promotion.description || promotion.banner_text}
        </p>

        {/* Countdown timer */}
        {promotion.countdown_enabled && promotion.end_date && (
          <div className="w-full max-w-md mx-auto mb-8 bg-neutral-900/60 p-4 rounded border border-white/5 backdrop-blur-sm">
        ...
        <div className="flex items-center justify-center gap-2 group-hover:translate-x-1 transition-transform">
          <span className="text-[11px] font-black uppercase tracking-[0.25em] text-white transition-colors">Aproveitar ofertas</span>
