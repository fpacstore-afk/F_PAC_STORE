...
const ManagementHome = lazyWithRetry(() => import('./pages/ManagementHome'));
const ManagementCatalog = lazyWithRetry(() => import('./pages/ManagementCatalog'));
...
function AppContent() {
  ...
  return (
    <main className={cn("flex-1", !isHome && "pt-[108px] md:pt-[130px]")}>
      <ErrorBoundary><Suspense fallback={<PageLoader />}><Routes>
        <Route path="/" element={<HomeV2 />} /><Route path="/catalog" element={<ProductCategories />} /><Route path="/catalog/all" element={<Catalog />} /><Route path="/produtos" element={<ProductCategories />} /><Route path="/produtos/:category" element={<ProductCategoryPage />} /><Route path="/estampas" element={<StampsGallery />} /><Route path="/galeria-estampas" element={<StampsGallery />} /><Route path="/prime" element={<PrimeCustomBuilder />} /><Route path="/prime-custom" element={<PrimeCustomBuilder />} /><Route path="/model/prime" element={<Navigate to="/prime" replace />} /><Route path="/model/:modelSlug" element={<ModelStamps />} /><Route path="/bag" element={<Bag />} /><Route path="/collections" element={<Navigate to="/catalog" replace />} /><Route path="/product" element={<Navigate to="/catalog" replace />} /><Route path="/produto" element={<Navigate to="/catalog" replace />} /><Route path="/product/:slug" element={<ProductDetail />} /><Route path="/produto/:slug" element={<ProductDetail />} /><Route path="/checkout" element={<Checkout />} /><Route path="/success" element={<SuccessPage />} /><Route path="/laboratorio-videos" element={<VideoSandbox />} /><Route path="/radio" element={<RadioPage />} /><Route path="/clube" element={<ClubeFPAC />} /><Route path="/clube-fpac" element={<ClubeFPAC />} /><Route path="/gestao" element={<ManagementHome />} /><Route path="/gestao/catalogo" element={<ManagementCatalog />} /><Route path="/gestao/operacoes" element={<AdminOrders />} /><Route path="/admin" element={<Navigate to="/gestao" replace />} /><Route path="/admin/estampas" element={<Navigate to="/gestao/operacoes" replace />} /><Route path="/admin/produtos" element={<Navigate to="/gestao/catalogo" replace />} /><Route path="/tracking" element={<OrderLookup />} /><Route path="/account" element={<Account />} /><Route path="/order/:orderId" element={<OrderStatus />} /><Route path="/order-status/:orderId" element={<NavigateToOrder />} /><Route path="*" element={<Navigate to="/" replace />} />
